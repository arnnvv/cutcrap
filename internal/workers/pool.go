...
import (
	...
	"pdf-processor/internal/transcript"
	"strings"
	"sync"
	"time"
)
...
func ProcessChunks(ctx context.Context, chunks []string, cfg *config.Config, ratio float64, mode string) []string {
	...
	// For transcript mode, combine results with transcript formatter
...
// New function for processing transcripts
func ProcessTranscript(ctx context.Context, text string, cfg *config.Config, ratio float64) string {
	...
	// Use space-based chunking for transcripts
	...
	// Process chunks in transcript mode
	...
	// Return the first (and only) result from transcript processing
	return results[0]
}
